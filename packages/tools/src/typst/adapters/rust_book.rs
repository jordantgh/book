...
use crate::typst::export::{PreparedMarkdown, TypstAdapter};
use crate::typst::paths::{
    ExportContext, GeneratedFiles, OutputLayout, read_text, relative_path,
};
...
impl TypstAdapter for RustBookAdapter {
    fn output_layout(&self) -> OutputLayout {
        OutputLayout {
            frontmatter_md: FRONTMATTER_MD.into(),
            body_md: BODY_MD.into(),
            frontmatter_typ: FRONTMATTER_TYP.into(),
            body_typ: BODY_TYP.into(),
            wrapper_typ: BOOK_TYP.into(),
            pdf: PDF_PATH.into(),
        }
    }
    ...
    fn prepare_markdown(
        ...
        let summary = read_text(&ctx.repo_root.join(SUMMARY_PATH))?;
        let source_paths = summary_paths(&summary);

        let title_page_path = ctx.repo_root.join(TITLE_PAGE_PATH);
        let title_page = read_text(&title_page_path)?;
        let frontmatter_source = strip_title_page_header(&title_page);
        let frontmatter =
            normalize_markdown(&frontmatter_source, &title_page_path, ctx)?;
        ...
        for relative in body_paths {
            let source_path = ctx.repo_root.join("src").join(relative);
            if !body.is_empty() {
                body.push_str("\n\n");
            }
            body.push_str(&normalize_source_file(&source_path, ctx)?);
        }

        Ok(PreparedMarkdown { frontmatter, body })
    }

    fn pandoc_filter(&self, ctx: &ExportContext) -> Option<PathBuf> {
        Some(ctx.repo_root.join(FILTER_PATH))
    }
...
fn normalize_source_file(
    ...
    normalize_markdown(&source, source_path, ctx)
}
...
fn normalize_markdown(
    ...
    let rewritten = rewrite_markdown(&expanded, source_path, ctx)?;
...
fn rewrite_markdown(
    ...
    while index < lines.len() {
        ...
        let stripped = INLINE_KBD
            ...
            .to_string();
        let stripped = replace_inline_images(&stripped, source_path, ctx)?;
    ...
    if let Some(image) = pending_image {
        output.push(markdown_image(&image.src, &image.alt, source_path, ctx)?);
    }
...
fn replace_inline_images_with_regex(
    ...
    for caps in regex.captures_iter(line) {
        ...
        output.push_str(&markdown_image(src, alt, source_path, ctx)?);
        last = whole.end();
    }
...
fn markdown_image(
    ...
    let escaped_path = relative_path(&ctx.output_dir, &image_path)?;
